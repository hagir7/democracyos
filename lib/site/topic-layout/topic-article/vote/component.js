...
const localeNumber = require('lib/utils').localeNumber
  ...
  render () {
    ...
    return (
      <div className='proposal-options topic-article-content'>
        ...
        {t('proposal-options.votes-cast', { num: localeNumber(votesTotal) })}
          </em>
        </div>
        {
          isRequired && (
      ...
      <div className='results-summary col-sm-6'>
        {
          Object.keys(votes)
            .map(function (v) {
              ...
              let s = votesByType.length === 1 || config.locale === 'fa' || config.locale === 'ar' ? '' : 's'

              return (
                <div className={option.className + ' votes-results'} key={v}>
                  <h5>{t(option.text)}</h5>
                  <span className='percent'>{localeNumber(width)}%</span>
                  <span className='votes'>
                    &nbsp;
                    {localeNumber(props.count)}
